...
| `basectl workspace update [--manifest] [--repos] [--dry-run] [--format json]` | Run `git pull --ff-only` across selected existing repositories in manifest order; emit a versioned JSON result report when requested |
